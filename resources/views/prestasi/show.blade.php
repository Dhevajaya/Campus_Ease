        ...
        <div class="card">
            <div class="card-body">
                <div class="text-center d-flex flex-column h-100">
                    <div>
                        <img src="{{ $result->avatar ? asset('storage/' . $result->avatar) : 'https://avatars.dicebear.com/api/initials/'. $result->name .'.png?background=blue' }}"
                                alt=""
    ...
    $(function(){
        ...
        $(document).on("click",".btn-delete",function(){
            if(confirm("Apakah anda yakin ingin menghapus data ini ?")){
                $("#frmDelete").attr("action", "{{ route('dashboard.users.destroy', 'id') }}".replace("id", '{{$result->id}}'));
                $("#frmDelete").submit();
            }
        })
        ...
        $(document).on("click",".btn-restore",function(){
            if(confirm("Apakah anda yakin ingin mengembalikan data ini ?")){
                $("#frmRestore").attr("action", "{{ route('dashboard.users.restore', 'id') }}".replace("id", '{{$result->id}}'));
                $("#frmRestore").submit();
            }
        })
    })
